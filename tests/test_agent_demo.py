...
def pytest_generate_tests(metafunc):
    ...
    if 'test_case' in metafunc.fixturenames:
        ...
        # 筛选Agent演示模块的测试用例
        agent_demo_cases = df[df['test_case_id'].str.startswith('TC02')]
        test_cases_list = agent_demo_cases.to_dict('records')
        
        # 生成参数
        metafunc.parametrize("test_case", test_cases_list, ids=[tc['test_case_id'] for tc in test_cases_list])


@allure.feature("Agent演示模块 (Agent Demo)")
class TestAgentDemo:
    """Agent演示模块测试类"""

    def test_agent_demo_api(self, api_client: APIClient, test_case: Dict[str, Any]):
        """
        Agent演示模块通用测试方法
        """
        # 设置allure标签
